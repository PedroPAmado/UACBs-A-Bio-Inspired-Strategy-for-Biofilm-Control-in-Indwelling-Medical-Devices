...
%of karlsruhe
%Altered by ARTORG Center: CVE group, Damiano Schirinzi
% Now it works also for frames without particles
...
posone=find(base>tl);
if isempty(posone) %The image is completely black (No pixels above threshold tl)
    %No particles found -> end function here
    i = [];
    j = [];
    return 
end
...
if ~(isempty(i) || isempty(j))
    % Filtering peaks that are close each other
    [i1,j1]=maxindomain(i,j,dim,base,sigma);           
